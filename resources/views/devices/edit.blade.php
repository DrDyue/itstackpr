    ...
    <section class="device-form-shell">
        <div class="device-page-header">
            <div>
                ...
                <p class="device-page-subtitle">Atjauno datus, foto un garantijas attelu.</p>
            </div>
        ...
        <form
            ...
            x-data="{ devicePreview: '{{ $device->deviceImageUrl() ?? '' }}', warrantyPreview: '{{ $device->warrantyImageUrl() ?? '' }}' }"
        >
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div class="device-upload-box">
                            ...
                            <input
                                ...
                                accept="image/*"
                                class="crud-control"
                                @change="devicePreview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : '{{ $device->deviceImageUrl() ?? '' }}'"
                            >
                        ...
                        <div class="device-upload-box">
                            ...
                            <input
                                ...
                                accept="image/*"
                                class="crud-control"
                                @change="warrantyPreview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : '{{ $device->warrantyImageUrl() ?? '' }}'"
                            >
