    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <form method="GET" action="{{ route('device-set-items.index') }}" class="mb-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('device-set-items.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <form method="POST" action="{{ route('device-set-items.destroy', $item) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Nonemt so poziciju?')" class="inline-flex items-center gap-2 text-red-600 hover:text-red-700">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
