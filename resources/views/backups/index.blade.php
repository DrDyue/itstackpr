<x-app-layout>
    @php
        ...
        $formatDateTime = function ($value, string $fallback = '-'): string {
            return $value ? $value->timezone(config('app.timezone'))->format('d.m.Y H:i') : $fallback;
        };
    ...
    <section class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex h-full flex-col justify-between rounded-[1.75rem] border border-sky-100 bg-sky-50 p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-3xl font-semibold text-slate-900">{{ $summary['count'] }}</p>
                    </div>
                </div>
                <div class="mt-5 flex flex-wrap gap-2 text-xs font-semibold">
                    <span class="rounded-full bg-white px-3 py-1.5 text-slate-600 ring-1 ring-sky-100">Manualas: {{ $summary['manual_count'] }}</span>
                    <span class="rounded-full bg-white px-3 py-1.5 text-slate-600 ring-1 ring-sky-100">Autom.: {{ $summary['scheduled_count'] }}</span>
                    <span class="rounded-full bg-white px-3 py-1.5 text-slate-600 ring-1 ring-sky-100">Importetas: {{ $summary['uploaded_count'] }}</span>
                </div>
            </div>
            ...
            <div class="flex h-full flex-col justify-between rounded-[1.75rem] border border-amber-100 bg-amber-50 p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-lg font-semibold leading-6 text-slate-900">{{ $formatDateTime($summary['latest']?->created_at, 'Nav izveidota') }}</p>
                    </div>
                </div>
                <p class="mt-5 rounded-2xl bg-white/80 px-3 py-2 text-sm text-slate-600 ring-1 ring-amber-100">
                    {{ $summary['latest'] ? $triggerLabel($summary['latest']->trigger_type) . ' | ' . $creatorLabel($summary['latest']) : 'Sagaida pirmo kopiju.' }}
                </p>
            </div>
            ...
            <div class="flex h-full flex-col justify-between rounded-[1.75rem] border border-violet-100 bg-violet-50 p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-3xl font-semibold text-slate-900">{{ $formatBytes($summary['total_size']) }}</p>
                    </div>
                </div>
            ...
            <div class="flex h-full flex-col justify-between rounded-[1.75rem] border border-emerald-100 bg-emerald-50 p-5 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-lg font-semibold leading-6 text-slate-900">{{ $summary['current']?->name ?? 'Nav fikseta' }}</p>
                    </div>
                </div>
                <p class="mt-5 rounded-2xl bg-white/80 px-3 py-2 text-sm text-slate-600 ring-1 ring-emerald-100">
                    {{ $summary['current']?->last_restored_at ? 'Atjaunota ' . $formatDateTime($summary['current']->last_restored_at) : 'Vel nav veikta atjaunosana.' }}
                </p>
            </div>
        </div>
        ...
        <div id="backup-history" class="mb-6 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div class="mb-5 flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h2 class="text-xl font-semibold text-slate-900">Rezerves kopiju vesture</h2>
                    <p class="mt-1 text-sm text-slate-500">Mekle un filtre pec nosaukuma, veida, autora un datuma.</p>
                </div>
            ...
            <form method="GET" action="{{ route('backups.index') }}" class="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_auto]">
                <label class="block">
                    <span class="user-filter-label">Meklet pec nosaukuma</span>
                    <div class="relative">
                        <svg xmlns="http://www.w3.org/2000/svg" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z"/></svg>
                        <input type="text" name="q" value="{{ $filters['q'] }}" placeholder="Piem. Manuala kopija vai datubaze" class="crud-control pl-10">
                    </div>
                </label>
                <label class="block">
                    <span class="user-filter-label">Veidotaja tips</span>
                    <select name="creator_scope" class="crud-control">
                        <option value="">Visi</option>
                        <option value="user" @selected($filters['creator_scope'] === 'user')>Lietotaji</option>
                        <option value="system" @selected($filters['creator_scope'] === 'system')>Sistema</option>
                    </select>
                </label>
                <label class="block">
                    <span class="user-filter-label">Veidotaja vards</span>
                    <input type="text" name="creator_name" value="{{ $filters['creator_name'] }}" placeholder="Piem. Janis Berzins" class="crud-control">
                </label>
                <label class="block">
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="crud-btn-primary inline-flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z"/></svg>
                        <span>Meklet</span>
                    </button>
                    <a href="{{ route('backups.index') }}" class="crud-btn-secondary inline-flex items-center gap-2">
                            ...
                            <tr class="transition hover:bg-slate-50">
                                <td class="px-4 py-4 align-top">
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-2 text-xs">
                                        <span class="{{ $triggerTone($backup->trigger_type) }} inline-flex rounded-full px-2.5 py-1 font-semibold">{{ $triggerLabel($backup->trigger_type) }}</span>
                                        <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 font-semibold text-slate-600">{{ strtoupper($backup->database_driver) }}</span>
                                        @if ($backup->database_name)
                                            <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 font-semibold text-slate-600">{{ $backup->database_name }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-4 align-top">
                                    <div class="font-semibold text-slate-900">{{ $creatorLabel($backup) }}</div>
                                    <div class="mt-1 text-sm text-slate-500">{{ $backup->trigger_type === 'uploaded' ? 'Fails no datora' : ($backup->trigger_type === 'scheduled' ? 'Automatiski izveidota' : 'Lietotaja izveidota') }}</div>
                                </td>
                                <td class="px-4 py-4 align-top">
                                    <div class="font-semibold text-slate-900">{{ $formatDateTime($backup->created_at) }}</div>
                                    <div class="mt-1 text-sm text-slate-500">{{ $formatBytes($backup->file_size_bytes) }} | {{ $formatDuration($backup->duration_ms) }}</div>
                                </td>
                                <td class="px-4 py-4 align-top">
                                    <div class="font-semibold text-slate-900">{{ $backup->total_tables }} tabulas</div>
                                    <div class="mt-1 text-sm text-slate-500">{{ number_format($backup->total_rows, 0, ',', ' ') }} ieraksti</div>
                                </td>
                                <td class="px-4 py-4 align-top">
                                    @if ($backup->is_current)
                                        <span class="inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Aktiva atjaunosana</span>
                                    @else
                                        <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">Arhivets snapshots</span>
                                    @endif
                                    <div class="mt-2 text-sm text-slate-500">{{ $backup->last_restored_at ? 'Pedeja atjaunosana ' . $formatDateTime($backup->last_restored_at) : 'Vel nav atjaunota' }}</div>
                                </td>
                                <td class="px-4 py-4 align-top">
                                    <div class="flex flex-wrap gap-2">
                                        <a href="{{ route('backups.download', ['backup' => $backup->id]) }}" class="inline-flex items-center gap-2 rounded-2xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-semibold text-sky-700 transition hover:bg-sky-100">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V4.5m0 12 4.5-4.5M12 16.5 7.5 12M4.5 19.5h15"/></svg>
                                            <span>Lejupieladet</span>
                                        </a>
                                        <form method="POST" action="{{ route('backups.restore', ['backup' => $backup->id]) }}">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100" onclick="return confirm('Atjaunot datubazi no si faila?')">
                                        ...
                                        <form method="POST" action="{{ route('backups.destroy', ['backup' => $backup->id]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700 transition hover:bg-rose-100" onclick="return confirm('Dzest so rezerves kopiju?')" @disabled($backup->is_current) title="{{ $backup->is_current ? 'Aktivu kopiju dzest nedrikst.' : 'Dzest kopiju' }}">
