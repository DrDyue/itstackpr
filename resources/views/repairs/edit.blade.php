<x-app-layout>
    ...
    <section class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('repairs.update', $repair) }}" class="crud-form-card">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $repair->start_date) }}" required class="crud-control">
                </div>
                <div>
                    <label class="crud-label">Plānotais beigums</label>
                    <input type="date" name="estimated_completion" value="{{ old('estimated_completion', $repair->estimated_completion) }}" class="crud-control">
                </div>
                <div>
                    <label class="crud-label">Faktiskais beigums</label>
                    <input type="date" name="actual_completion" value="{{ old('actual_completion', $repair->actual_completion) }}" class="crud-control">
                </div>
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="vendor_name" maxlength="100" value="{{ old('vendor_name', $repair->vendor_name) }}" class="crud-control">
                </div>
                <div>
                    <label class="crud-label">Piegādātāja kontakts</label>
                    <input type="text" name="vendor_contact" maxlength="100" value="{{ old('vendor_contact', $repair->vendor_contact) }}" class="crud-control">
                </div>
            </div>
